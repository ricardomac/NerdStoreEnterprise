...
namespace NSE.Carrinho.API.Controllers
{
    ...
    public class CarrinhoController : MainController
    {
        ...
        private void ManipularCarrinhoExistente(CarrinhoCliente carrinhoCliente, CarrinhoItem item)
        {
            ...
            {
                _context.CarrinhoItens.Update(carrinhoCliente.ObterPorProdutoId(item.ProdutoId));
            }
            else
            {
        ...
        private async Task<CarrinhoItem> ObterItemCarrinhoValidado(Guid produtoId, CarrinhoCliente carrinhoCliente,
            CarrinhoItem item = null)
        {
            ...
            var itemCarrinho = await _context.CarrinhoItens.FirstOrDefaultAsync(i =>
                i.CarrinhoId == carrinhoCliente.Id && i.ProdutoId == produtoId);
        ...
        private bool ValidarCarrinho(CarrinhoCliente carrinhoCliente)
        {
            if (carrinhoCliente.EhValido()) return true;

            carrinhoCliente.ValidationResult.Errors.ToList().ForEach(e => AdicionarErroProcessamento(e.ErrorMessage));
            return false;
        }
    }
}